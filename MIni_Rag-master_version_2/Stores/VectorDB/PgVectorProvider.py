...
from .Providers.VectoDBEnums import (DistancMethodEnums, PgVectorTablesSchemeEnums, 
...
class PGVectorProvider(VectorDBInterface):
    ...
    def __init__(self, db_client, default_vector_size: int = 786,
        ...
        if distance_method == DistancMethodEnums.COSINE.value:
            distance_method = PgVectorDistanceMethodEnums.COSINE.value
        elif distance_method == DistancMethodEnums.DOT.value:
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                create_idx_sql = sql_text(
                                            ...
                                            f'USING {index_type} ({PgVectorTablesSchemeEnums.VECTOR.value} {self.distance_method})'
                                          )
        ...
        async with self.db_client() as session:
            async with session.begin():
                insert_sql = sql_text(f'INSERT INTO {collection_name} '
                                      f'({PgVectorTablesSchemeEnums.TEXT.value}, {PgVectorTablesSchemeEnums.VECTOR.value}, {PgVectorTablesSchemeEnums.METADATA.value}, {PgVectorTablesSchemeEnums.CHUNK_ID.value}) '
    ...
    async def insert_many(self, collection_name: str, texts: list,
        ...
        if len(texts) != len(vectors) or len(vectors) != len(record_ids):
            ...
            async with session.begin():
                for i in range(0, len(texts), batch_size):
                    ...
                    batch_insert_sql = sql_text(f'INSERT INTO {collection_name} '
                                    f'({PgVectorTablesSchemeEnums.TEXT.value}, '
                                    f'{PgVectorTablesSchemeEnums.VECTOR.value}, '
                                    f'{PgVectorTablesSchemeEnums.METADATA.value}, '
                                    f'{PgVectorTablesSchemeEnums.CHUNK_ID.value}) '
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int):
        ...
        async with self.db_client() as session:
            async with session.begin():
                search_sql = sql_text(f'SELECT {PgVectorTablesSchemeEnums.TEXT.value} as text, '
                                      f'1 - ({PgVectorTablesSchemeEnums.VECTOR.value} <=> :vector) as score, '
                                      f'{PgVectorTablesSchemeEnums.METADATA.value} as metadata, '
                                      f'{PgVectorTablesSchemeEnums.CHUNK_ID.value} as chunk_id'
                                      f' FROM {collection_name}'
                                      ' WHERE 1 - (vector <=> :vector) > 0.1'  # Lower threshold for more results
                                      ' ORDER BY score DESC '
                                      f'LIMIT {limit}'
                                      )
                ...
                self.logger.info(f"Found {len(records)} results for search in {collection_name}")
                for i, record in enumerate(records[:3]):  # Log top 3 results
                    self.logger.info(f"Result {i+1}: score={record.score:.3f}, text_length={len(record.text)}")
                ...
                return [
                    RetrievedDocument(
                        ...
                        asset_id=(record.metadata.get('asset_id') if record.metadata else None),
